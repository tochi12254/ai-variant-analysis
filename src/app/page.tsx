        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center space-x-2">
            <div className="relative">
              <h1 className="text-xl font-light tracking-wide text-[#364f3d]">
                <span className="font-normal">Va</span>
                <span className="text-[#de8246]">rion</span>
              </h1>
            ...
            <CardContent className="pb-4">
                  ...
                  <div className="space-y-4">
                    <form onSubmit={handleSearch} className="flex flex-col gap-3 sm:flex-row">
                        <div className="relative flex-1">
                          <Input className="h-9 border-[#3c4f3d]/10 pr-10" type="text"  placeholder="Enter gene symbol or name" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                          <Button className="absolute top-0 right-0 h-full cursor-pointer rounded-l-none bg-[#3c4f3d] text-white hover:bg-[#3c4f3d]/90" type="submit" size="icon" disabled={isLoading || !searchQuery.trim()}>
                            <Search className="h-4 w-4" />
                            <span className="sr-only">Search</span>
                          </Button>
                        </div>
                    </form>
                    <Button variant="link" className="h-auto cursor-pointer p-0 text-[#de8246] hover:text-[#de8246]/80" onClick={loadBRCA1Example}>Try BRCA1 example</Button>
                  </div>
                </TabsContent>
                ...
                <TabsContent value="browse" className="mt-0">
                  <div className="max-h-[150px] overflow-y-auto pr-1">
                    <div className="flex flex-wrap gap-2">
                        ...
                        <Button key={chrom.name} variant="outline" size="sm" className={`h-8 cursor-pointer border-[#3c4f3d]/10 hover:bg-[#e9eeea] hover:text-[#3c4f3d] ${selectedChromosome === chrom.name ? "bg-[#e9eeea] text-[#3c4f3d]" : ""}`} onClick={() => setSelectedChromosome(chrom.name)}>
                          {chrom.name}
                        </Button>)}
                    </div>
                  </div>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-[#e9eeea]/50 hover:bg-[#e9eeea]/70">
                          <TableHead className="text-sm font-normal text-[#3c4f3d]/70">Symbol</TableHead>
                          <TableHead className="text-sm font-normal text-[#3c4f3d]/70">Name</TableHead>
                          <TableHead className="text-sm font-normal text-[#3c4f3d]/70">Location</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {searchResults.map((gene, idx) => (
                          <TableRow key={`${gene.symbol}-${idx}`} className="cursor-pointer border-b border=[#3c4f3d]/5 hover:bg-[#e9eeea]/50" onClick={() => setSelectedGene(gene)}>
              ...
              {!isLoading && !error && searchResults.length === 0 &&  (
                <div className="flex h-40 flex-col items-center justify-center text-center text-gray-400">
                  ...
                  <p className="text-sm leading-relaxed">{mode === "search" ? "Enter a gene or symbol and click search" : selectedChromosome ? "No genes found on this chromosome" : "Select a chromosome to view genes"}</p>
                </div>
              )}
            </CardContent>
          </Card>
