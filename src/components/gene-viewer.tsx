...
export default function GeneViewer({gene, genomeId, onClose} : GeneViewerProps) {
  ...
  const variantAnalysisRef = useRef<VariantAnalysisHandle>(null);
    ...
    const fetchedGeneSequence = useCallback(
       ...
       const initializeGeneData = async () => {
        ...
        try {
            ...
            if (fetchedRange) {
              ...
              await fetchedGeneSequence(fetchedRange.start, fetchedRange.end)
            }
        } catch (error) {
    ...
    const handleLoadSequence = useCallback(() => {
        ...
        setError(null);
        fetchedGeneSequence(start, end);
      }, [startPosition, endPosition, apiFetchGeneSequence, geneBounds]);
    ...
    return (
        <div className="space-y-6">
          <Button variant="ghost" size="sm" className="cursor-pointer text-[#3c4f3d] hover:bg-[#e9eeea]/70" onClick={onClose}>
        ...
        />
          ...
          <GeneInfo gene={gene} geneDetail={geneDetail} geneBounds={geneBounds} />
