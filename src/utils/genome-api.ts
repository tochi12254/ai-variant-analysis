...
export async function getAvailableGenomes() {
    ...
    const genomeData = await res.json()
...
export async function getGenomeChromosomes(genomeId: string) {
   ...
   const chromosomeData = await res.json()
    ...
    for (const chromosomeId in chromosomeData.chromosomes) {
        if (chromosomeId.includes("_") || chromosomeId.includes("Un") || chromosomeId.includes("random")) continue;
        chromosomes.push({
            name: chromosomeId,
...
export async function fetchGeneSequence(chrom: string, start: number, end: number, genomeId: string): Promise<{sequence: string; actualRange: {start: number; end: number}; error?: string;}> {
    try {
        ...
        return { sequence: "", actualRange: { start, end }, error: "Internal error in fetching gene sequence" };
    }
}
